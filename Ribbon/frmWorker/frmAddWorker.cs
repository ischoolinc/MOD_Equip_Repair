...
{
    public partial class frmAddWorker : BaseForm
    {
        public frmAddWorker()
        {
            InitializeComponent();
        ...
        private void dataGridViewX1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                if (e.ColumnIndex == 5)
                {
                    ...
                    string roleID = Program._workerRoleID;
                    string userAccount = DAO.Actor.Instance.GetUserAccount();

                    if (string.IsNullOrEmpty(account))
                    {
                        MsgBox.Show(string.Format("{0}教師沒有登入帳號，無法指定為設施報修維修員!", teacherName));
                        return;
                    }
                    else
                    {
                        DialogResult result = MsgBox.Show(string.Format("確定將{0}教師指定為設施報修維修員?", teacherName), "提醒", MessageBoxButtons.YesNo);
                        if (result == DialogResult.Yes)
                        {
                            try
                            {
                                DAO.Worker.InsertWorker(teacherID, account, loginID, roleID, DateTime.Now.ToString("yyyy/MM/dd"), userAccount);
                                MsgBox.Show("資料新增成功!");
